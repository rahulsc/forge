...
|---|---------|-------|----------|--------|----------|------------|------------|--------|
| 1 | v0.2.0-dev | `brainstorming` | Design doc written to `docs/<project>/design/design.md` | Skill hardcodes `docs/plans/<project>/design.md` — wrong directory structure | friction | Skill text uses incorrect path convention | v0.2.0 | fixed (v0.2.0) |
| 2 | v0.2.0-dev | repo structure | Product artifacts at top level (like `agents/`, `skills/`, `hooks/`) | Product tools (`bin/`, `policies/`, `adapters/`, `workflows/`, `packs/`) nested inside `.forge/`, mixing product infra with per-project config | friction | Inherited from upstream; `.forge/` was treated as a catch-all | v0.2.0 | fixed (v0.2.0) |
| 3 | v0.2.0-dev | `setting-up-project` | Elevated tier requires worktree creation | Skipped worktree — developing Beta directly on main with backup branch `forge-v0.1.1-baseline` instead. Worktree adds complexity without benefit when there are no external users. | cosmetic | Skill assumes all elevated work needs isolation; dogfooding on own repo is a special case | v0.3.0 | closed (v0.3.0 — execution ergonomics made worktrees optional) |
| 4 | v0.2.0-dev | `setting-up-project` | When team criteria met, invoke `forge:composing-teams` for structured team composition with specialist recommendations | Skipped composing-teams entirely. Set `team.decision = team` and improvised team structure during plan execution. No agent-to-task mapping was presented, no specialist suggestions, no user review of team composition. | friction | Lead skipped the composition step, likely because the skill says "invoke composing-teams" but there was no enforcement gate. The composing-teams skill should present agent options and get user approval before planning. | v0.3.0 | fixed (v0.2.1) |
| 5 | v0.2.0-dev | `agent-team-driven-development` | Two-stage review (spec + code quality) dispatched as subagents per task | Skipped subagent reviews for Wave 0. Tasks were mechanical (find-replace, grep-verified) — spawning 4 review subagents for 14 line changes would be pure overhead. | cosmetic | Review ceremony doesn't scale down for mechanical tasks. Risk-tier-aware review should allow lighter ceremony when verification is grep-based and evidence is clear. | v0.3.0 | fixed (v0.3.0 — review tiers) |
| 6 | v0.2.0-dev | `agent-team-driven-development` | Clean, minimal commit history | Multi-agent execution produces excessive commits (40-50 for complex features). Each agent working commit pollutes git history. | friction | No squash-on-merge strategy exists. Agents commit freely in worktrees and all commits land on main. Need one commit per complete change. | v0.3.0 | fixed (v0.2.1) |
| 7 | v0.2.0-dev | `agent-team-driven-development` | Completed tasks and shutdown agents show clean state — no stale `in_progress` tasks, no orphaned agents | After Wave 0, T1/T2 tasks and agent tasks (#8/#9) remained `in_progress` despite work being done and merged. Shutdown requests were sent but task status wasn't fully cleaned up. Lead had to manually reconcile. | friction | Task cleanup sequence is fragile. Shutdown + TaskUpdate + agent-specific task cleanup need to be atomic or at least reliably sequenced. The skill says to clean up tasks before shutdown but this didn't happen cleanly. | v0.3.0 | fixed (v0.2.1) |
...
| 9 | v0.2.0–v0.3.0 | `writing-plans`, `agent-team-driven-development` | Pipelined TDD or solo TDD used where applicable | TDD was not exercised during Beta development. All tasks were documentation/skill edits where tests weren't applicable, but TDD enforcement is a core Forge differentiator that must work for all projects using Forge. | friction | Tasks so far have been markdown-only. Going forward, Forge must enforce interleaved TDD where possible — meaning wherever testable code or verifiable behavior exists. Don't force meaningless tests, but don't skip TDD when it applies. | v0.3.1 | fixed (v0.3.1) |
| 10 | v0.3.1-dev | `setting-up-project` | Risk classification and team decision presented to user for confirmation before proceeding | Lead auto-approved both risk tier (standard) and team decision (solo) without asking the user. Step 1 says "Ask: Does this classification look correct?" and Step 4 should similarly confirm the team decision with the user before writing to state. | friction | Lead skipped confirmation prompts to move faster. The skill text says to ask but doesn't enforce it with a HARD-GATE. Both risk and team decisions should require explicit user confirmation. | v0.3.1 | fixed (v0.3.1) |
| 11 | v0.3.1-dev | `setting-up-project` | Risk classification should include justification, and team criteria evaluation should be presented alongside risk (Step 1) rather than as a separate Step 4 | Risk tier presented without justification ("standard" with no reasoning). Team decision happens in Step 4 but it's really part of the same classification — team-optional/recommended/required is derived from risk tier + scope. These should be one step with full reasoning shown upfront. | friction | Steps 1 and 4 are artificially separated. The team decision table is the justification for why the strategy is team-optional/solo/team — it should be presented with the risk classification so the user sees the full picture at once. | v0.3.1 | fixed (v0.3.1) |
