...
| 21 | v1.0 | reference project workflow | Runtime isolation per reference project (nvm for Node.js, pyenv for Python) | npm install runs in the worktree but uses the system Node.js version. Different reference projects may need different runtime versions. No isolation between projects sharing the same host runtime. | cosmetic | For v1.0: scaffold scripts should set up local runtime via nvm/pyenv/similar. Each worktree gets its own Node/Python version. Prevents version conflicts between reference projects. | v1.0 | open |
| 22 | v0.4.4-ref | `adopting-forge` | Detect prior orchestration systems (.superpowers/, .aider/, .copilot-workspace/) | Skill detects AI surfaces (CLAUDE.md, .cursor/) but missed .superpowers/ — a prior orchestration state directory. Should scan for common orchestration dirs and offer migration/cleanup. | friction | Skill only looks for Forge-native and common AI files, not competing orchestration systems. | v0.5.0 | open |
| 23 | v0.4.4-ref | `adopting-forge` | Detect existing design docs and plans | Project had 7 completed phases of docs in docs/plans/ — skill never noticed. Should report existing design/plan artifacts and register with forge-state. | friction | Skill focuses on config detection, not project history/documentation discovery. | v0.5.0 | open |
| 24 | v0.4.4-ref | `adopting-forge` | Update root .gitignore for replaced orchestration systems | Didn't update root .gitignore — old .superpowers/ entry remained, .forge/local/ missing. Should check root .gitignore for stale entries and add .forge/local/. | friction | Skill creates .forge/local/.gitignore but doesn't touch project root .gitignore. | v0.5.0 | open |
| 25 | v0.4.4-ref | `adopting-forge` | Offer cleanup of stale orchestration state | No prompt to remove .superpowers/ after detecting it. Should ask: "Found .superpowers/ (prior system). Remove it?" | friction | No foreign state cleanup logic exists. | v0.5.0 | open |
| 26 | v0.4.4-ref | `adopting-forge`, `brainstorming` | Project conventions auto-detected and recorded | Agents don't follow project conventions (test import patterns, CSS variable usage) because conventions aren't recorded in .forge/shared/conventions.md during adoption or brainstorming. forge:adopting-forge or forge:brainstorming should detect patterns (test setup, imports, linting config, design tokens) and write them to conventions.md so all agents follow them. | friction | No convention auto-detection exists. Agents start from scratch each time. | v0.5.0 | open |
