...
| 14 | v0.4.1-dev | multiple skills | Skill outputs use consistent, readable formatting (tables, structured sections) | Several skills produce wall-of-text output where tables would be clearer: setting-up-project assessment, readiness summary, status reports, adopting-forge step 2 output. Format quality varies across skills. Specific adopting-forge issue: Step 2 proposed configuration uses aligned text blocks instead of tables; risk areas lack icons and legend explaining what each tier means. | cosmetic | No output formatting guidance in skills — each invocation improvises. Need a formatting audit across all skill outputs and standardized guidance. | v0.5.0 | fixed (v0.5.0 — formatting audit complete, forge-author quality checklist updated with table/icon rules) |
| 15 | v0.4.1-dev | `agent-team-driven-development` | Multi-agent work progress always visible during execution | Task list only visible via Ctrl+T toggle (ephemeral). Users lose visibility after next output. | cosmetic | Lead should print formatted task list: (1) after each major action (task completion, agent spawn, between waves), and (2) on a timer — if no output for 30 seconds, print current task status. Skill convention, not platform feature. | v0.5.0 | fixed (v0.4.2 — wave-grouped format with icons, 30s timer) |
| 16 | v0.4.1-ref | `agent-team-driven-development` | Agents report completion and shut down without manual intervention | Agents get stuck in "standing by" loop at two points: (1) pre-report — work is done but agent doesn't send the report until Escape, (2) post-shutdown-request — shutdown sent but agent cycles idle. Both require manual Esc to unstick. | friction | Likely a Claude Code platform behavior. Mitigation for v0.5.0: lead sends timed check messages — first a nudge to the agent ("are you stuck?"), then if still no response, escalate to the user ("Agent <name> appears stuck — please check its pane and press Esc if it's cycling idle"). Report timeout: 60s. Shutdown timeout: 30s. | v0.5.0 | mitigated (v0.5.0 — timed nudge + user escalation added to skill; Esc workaround documented in usage guide. Platform behavior can't be fully fixed.) |
| 17 | v0.4.1-ref | audit infrastructure | Audit JSONL captures events during reference project runs | Only session_start recorded at repo root. No skill/task/gate events captured. Agents weren't instructed to call forge-audit. Hooks only fire at session level, not per-agent. | blocker | Two fixes needed: (1) agents must explicitly call `bin/forge-audit record` at task boundaries, (2) events record at repo root audit log (acceptable — don't need per-subfolder). | v0.4.2 | fixed (v0.4.2 — audit recording in agent prompt template) |
| 18 | v0.4.1-ref | reference project workflow | User never asked to validate the output project works | No user validation checklist provided. No curl commands, no "try this endpoint" guide. Agent-reported "50 tests pass" was never independently verified. | blocker | Each reference project needs a user validation checklist: specific commands to run, expected outputs, pass/fail criteria. Show at start of run alongside approval manifest. | v0.4.2 | fixed (v0.4.2 — validation.md + HARD-GATE in finishing skill) |
| 19 | v0.4.1-ref | reference project workflow | Every approval is predictable and should be pre-approved once | User had to approve routine operations (file creates, npm install, npm test, git commit) repeatedly across agents. All were within the project directory, all were predictable from the spec. | friction | Each reference project spec should include an approval manifest shown at start of run. User approves once, only genuinely new/unexpected requests prompt during the run. | v0.4.2 | fixed (v0.4.2 — approvals.yaml integration in agent-team-driven-development) |
| 20 | v0.4.1-ref | reference project workflow | Run results and deviation fixes committed to main properly | Deviation fixes discovered during reference run (formatting, task list) not committed to main. Results file left in worktree. Process unclear for what goes to main vs stays in worktree. | friction | Process: (1) deviations/fixes commit to main immediately when discovered, (2) run results cherry-picked to main after run, (3) worktree deleted after results extracted. | v0.4.2 | fixed (v0.4.2 — results process in runbook) |
| 21 | v1.0 | reference project workflow | Runtime isolation per reference project (nvm for Node.js, pyenv for Python) | npm install runs in the worktree but uses the system Node.js version. Different reference projects may need different runtime versions. No isolation between projects sharing the same host runtime. | cosmetic | For v1.0: scaffold scripts should set up local runtime via nvm/pyenv/similar. Each worktree gets its own Node/Python version. Prevents version conflicts between reference projects. | v1.0 | deferred (v1.0 — not needed for Beta; system runtime works for current reference projects) |
| 22 | v0.4.4-ref | `adopting-forge` | Detect prior orchestration systems (.superpowers/, .aider/, .copilot-workspace/) | Skill detects AI surfaces (CLAUDE.md, .cursor/) but missed .superpowers/ — a prior orchestration state directory. Should scan for common orchestration dirs and offer migration/cleanup. | friction | Skill only looks for Forge-native and common AI files, not competing orchestration systems. | v0.5.0 | fixed (v0.5.0 — prior orchestration detection table + cleanup prompt added to adopting-forge Step 1) |
| 23 | v0.4.4-ref | `adopting-forge` | Detect existing design docs and plans | Project had 7 completed phases of docs in docs/plans/ — skill never noticed. Should report existing design/plan artifacts and register with forge-state. | friction | Skill focuses on config detection, not project history/documentation discovery. | v0.5.0 | fixed (v0.5.0 — design doc discovery scan added to adopting-forge Step 1) |
| 24 | v0.4.4-ref | `adopting-forge` | Update root .gitignore for replaced orchestration systems | Didn't update root .gitignore — old .superpowers/ entry remained, .forge/local/ missing. Should check root .gitignore for stale entries and add .forge/local/. | friction | Skill creates .forge/local/.gitignore but doesn't touch project root .gitignore. | v0.5.0 | fixed (v0.5.0 — root .gitignore update checklist added to adopting-forge Step 5) |
| 25 | v0.4.4-ref | `adopting-forge` | Offer cleanup of stale orchestration state | No prompt to remove .superpowers/ after detecting it. Should ask: "Found .superpowers/ (prior system). Remove it?" | friction | No foreign state cleanup logic exists. | v0.5.0 | fixed (v0.5.0 — cleanup prompt included in #22 prior orchestration detection) |
| 26 | v0.4.4-ref | `adopting-forge`, `brainstorming` | Project conventions auto-detected and recorded | Agents don't follow project conventions (test import patterns, CSS variable usage) because conventions aren't recorded in .forge/shared/conventions.md during adoption or brainstorming. forge:adopting-forge or forge:brainstorming should detect patterns (test setup, imports, linting config, design tokens) and write them to conventions.md so all agents follow them. | friction | No convention auto-detection exists. Agents start from scratch each time. | v0.5.0 | fixed (v0.5.0 — convention detection added to both brainstorming Step 1 and adopting-forge Step 2) |
| 27 | v0.5.0-dev | `adopting-forge` | Storage backend choice offered during adoption | Forge defaulted to JSON storage without asking. Should detect if sqlite3 is available and offer the choice: "SQLite detected. Use SQLite (recommended) or JSON for state storage?" | friction | No storage backend selection in adoption flow. forge-state init picks JSON if sqlite3 not detected, but doesn't ask when sqlite3 IS available. | v0.5.0 | fixed (v0.5.0) |
