...
cat > "$TMPDIR_TEST/policies/default.yaml" <<'YAML'
rules:
  - match: ["src/**"]
    tier: standard
YAML
...
if grep -q "src/\*\*" "$TMPDIR_TEST/policies/default.yaml" 2>/dev/null; then
...
if [ -f "$TMPDIR_TEST/policies/greeting-policy.yaml" ]; then
...
if grep -q "match:" "$TMPDIR_TEST/policies/greeting-policy.yaml" 2>/dev/null; then
